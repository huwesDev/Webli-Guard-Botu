...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const sunucuGun = member?.joinedTimestamp ? Math.floor((Date.now() - member.joinedTimestamp) / 86400000) : null;
    const roller    = member ? member.roles.cache.filter(r => r.id !== interaction.guild.id).sort((a, b) => b.position - a.position) : null;
    const rolStr    = roller?.size ? roller.first(12).map(r => `<@&${r.id}>`).join(' ') + (roller.size > 12 ? ` +${roller.size - 12}` : '') : 'Yok';
    ...
    await interaction.reply({
      ...
      components: [
        new ContainerBuilder().setAccentColor(member?.displayColor || COLOR.brand)
          ...
          .addTextDisplayComponents(
            txt([
              ...
              sunucuGun !== null ? `**Sunucuda:** ${sunucuGun} gün  •  <t:${Math.floor(member.joinedTimestamp / 1000)}:D>` : null,
