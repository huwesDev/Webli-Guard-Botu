  ...
  async execute(interaction) {
    ...
    try {
      await user.send({
        ...
        components: [dmContainer(interaction.guild.name, interaction.guild.iconURL(), 'kick', sebep, interaction.user.tag)],
      }).catch(() => {});
      ...
      addLog(interaction.guild.id, 'ban', `Kick: ${user.tag}`, `Sebep: ${sebep}`,
        interaction.user.id, interaction.user.tag, user.id, user.tag, '#f59f00');

      await interaction.reply({
        flags: MessageFlags.IsComponentsV2,
        components: [modAction('👢  Kullanıcı Atıldı', COLOR.warn, user, interaction.user, sebep)],
      });
    } catch (e) {
