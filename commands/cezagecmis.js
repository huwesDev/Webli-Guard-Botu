...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const listText = gecmis.length
      ? gecmis.map((c, i) => `**${i + 1}.** ${TIP_EMOJI[c.tip] ?? '📋'} \`${c.tip.toUpperCase()}\`  ${c.sebep}\n> ${relTime(c.tarih)}  •  <@${c.mod_id}>`).join('\n\n')
      : '> Ceza geçmişi temiz. ✅';

    await interaction.reply({
      flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      components: [
        new ContainerBuilder().setAccentColor(gecmis.length ? COLOR.danger : COLOR.success)
