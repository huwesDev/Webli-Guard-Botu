const { SlashCommandBuilder, PermissionFlagsBits, ContainerBuilder, MessageFlags, ChannelType } = require('discord.js');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'create') {
      ...
      const roller  = guild.roles.cache.filter(r => r.id !== guild.id).map(r => ({ name: r.name, color: r.color, hoist: r.hoist, permissions: r.permissions.bitfield.toString(), mentionable: r.mentionable, position: r.position }));
      const kanallar = guild.channels.cache.map(c => ({ id: c.id, name: c.name, type: c.type, parentId: c.parentId, position: c.position, topic: c.topic || null }));
      ...
      return interaction.editReply({
        ...
        components: [
          new ContainerBuilder().setAccentColor(COLOR.success)
            .addTextDisplayComponents(txt('### 📦  Yedek Oluşturuldu'))
    ...
    if (sub === 'liste') {
      ...
      const listText = yedekler.length
        ? yedekler.map((b, i) => `**${i + 1}.** \`${b.id}\`  —  **${b.isim}**\n> <t:${Math.floor(b.tarih / 1000)}:D>  ${relTime(b.tarih)}`).join('\n\n')
        : '> Henüz yedek oluşturulmamış.';
      return interaction.reply({
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        components: [
          new ContainerBuilder().setAccentColor(COLOR.brand)
            .addTextDisplayComponents(txt(`### 📦  Yedek Listesi`))
            .addSeparatorComponents(sep())
            .addTextDisplayComponents(
              txt(listText),
              txt(`-# Toplam: ${yedekler.length} yedek  •  ${timestamp()}`),
            ),
        ],
      });
    }
    ...
    if (sub === 'yukle') {
      ...
      for (const r of [...roller].sort((a, b) => a.position - b.position)) {
        ...
        try { await guild.roles.create({ name: r.name, color: r.color, hoist: r.hoist, mentionable: r.mentionable, permissions: BigInt(r.permissions), reason: '[Guard] Backup' }); rOk++; }
        catch { rErr++; }
      }
      ...
      for (const k of kanallar.filter(c => c.type === ChannelType.GuildCategory)) {
        ...
        if (!mevcut) {
          try { const y = await guild.channels.create({ name: k.name, type: ChannelType.GuildCategory, reason: '[Guard] Backup' }); catMap.set(k.name, y.id); kOk++; }
          catch { kErr++; }
      ...
      for (const k of kanallar.filter(c => c.type !== ChannelType.GuildCategory)) {
        ...
        try {
          let parentId = null;
          if (k.parentId) { const kat = kanallar.find(c => c.id === k.parentId); if (kat) parentId = catMap.get(kat.name) || null; }
          await guild.channels.create({ name: k.name, type: k.type, parent: parentId, topic: k.topic || null, reason: '[Guard] Backup' }); kOk++;
        } catch { kErr++; }
      }
      ...
      return interaction.editReply({
        ...
        components: [
          new ContainerBuilder().setAccentColor(COLOR.success)
            .addTextDisplayComponents(txt('### 📦  Yedek Yüklendi'))
            .addSeparatorComponents(sep())
            .addTextDisplayComponents(
              txt(`**Yedek:** ${yedek.isim} \`(${id})\`\n**Rol:** ✅ ${rOk}  ❌ ${rErr}\n**Kanal:** ✅ ${kOk}  ❌ ${kErr}`),
              txt(`-# ${timestamp()}`),
            ),
        ],
      });
